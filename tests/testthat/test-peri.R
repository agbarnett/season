test_that("peri returns the documented periodogram outputs", {
  ...
  expect_named(
    peri_output,
    c("peri", "freq_radians", "freq_cycles", "amp", "phase")
  )
  ...
  expect_true(is.na(peri_output$freq_cycles[1]))
  expect_identical(peri_output$freq_radians[1], 0)
})
...
test_that("peri's largest periodogram peak lands on the seeded cycle length", {
  ...
  peri_output <- peri(x)
  non_na <- which(!is.na(peri_output$freq_cycles))
  peri_non_missing <- peri_output$peri[non_na]
  peri_max_idx <- which.max(peri_non_missing)
  expect_identical(
    peri_output$freq_cycles[non_na[peri_max_idx]],
    period_12
  )
})
...
test_that("peri recovers the phase of a pure cosine at its peak frequency", {
  ...
  expect_identical(peri_output$freq_cycles[peak], period)
