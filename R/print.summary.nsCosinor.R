...
print.summary.nscosinor <- function(x, ...) {
  ...
  if (k == 1) {
    ...
    cat(
      "Phase (radians), mean=",
      x$stats$phasestats[1],
      ", 95% CI [",
      x$stats$phasestats[2],
      ", ",
      x$stats$phasestats[3],
      "]\n",
      sep = "",
      ...
    )
  ...
  if (k >= 2) {
    for (index in 1:k) {
      ...
      cat(
        "Season, mean=",
        x$stats$wstats[index, 1],
        ", 95% CI [",
        x$stats$wstats[index, 2],
        ", ",
        x$stats$wstats[index, 3],
        "]\n",
        sep = "",
        ...
      )
    ...
    for (index in 1:k) {
      ...
      cat(
        "Amplitude, mean=",
        x$stats$ampstats[index, 1],
        ", 95% CI [",
        x$stats$ampstats[index, 2],
        ", ",
        x$stats$ampstats[index, 3],
        "]\n",
        sep = "",
        ...
      )
      cat(
        "Phase (radians), mean=",
        x$stats$phasestats[index, 1],
        ", 95% CI [",
        x$stats$phasestats[index, 2],
        ", ",
        x$stats$phasestats[index, 3],
        "]\n",
        sep = "",
        ...
      )
